import lifeint as lifeint

__doc__ = """Pattern construction for Conway's Game of Life.""";
...
It allows to access the pattern's phases as elements of an array."""
	...
	def __call__ (self, x, y, A = identity):
		...
		return pattern (lifeint.transform (self, x, y, *A))
...
rcw (rotate clockwise) and rccw (rotate counter-clockwise)."""
	...
	def put (self, x = 0, y = 0, A = identity):
		"""Send transformed pattern to stdout in Life 1.06 format."""
		lifeint.put (self, x, y, *A)
		return None
...
It is also the base for computing generations subsequent to N-th."""
		...
		for k in self.__phases.keys ():
			if M < k < N: M = k
		p = self.__phases[N] = pattern (lifeint.evolve (self.__phases[M], N - M))
		return p
...
"""
		...
		if type (P) == list:
			...
			list.__init__ (self, lifeint.parse (P, x0, y0, *A))
		else:
...
def load (fn):
	return pattern (lifeint.load (fn))
